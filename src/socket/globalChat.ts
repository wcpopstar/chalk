...
function registerGlobalChatHandlers(io: any, socket: any, userId: any) {
  secureOn(io, socket, userId, 'global:message', async ({ text }: any, ack: any) => {
  ...
  secureOn(io, socket, userId, 'global:gif', async ({ gifUrl }: any, ack: any) => {
    const msg = await saveGlobalMessage({ senderId: userId, type: 'gif', mediaUrl: gifUrl });
    io.to('global').emit('global:message', msg);
    ack({ ok: true });
  });

  secureOn(io, socket, userId, 'global:voice', async ({ audio, mime, duration }: any, ack: any) => {
  ...
  secureOn(io, socket, userId, 'global:video_note', async ({ video, mime, duration }: any, ack: any) => {
  ...
  secureOn(io, socket, userId, 'global:edit', async ({ messageId, text }: any, ack: any) => {
  ...
  secureOn(io, socket, userId, 'global:delete', async ({ messageId }: any, ack: any) => {
