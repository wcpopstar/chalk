...
const definition = {
  ...
  info: {
    ...
    description:
      'REST API for the Chalk backend: auth, profiles, friends, chats (direct/group/global), ' +
  ...
  components: {
    securitySchemes: {
      bearerAuth: {
        ...
        bearerFormat: 'JWT',
        description:
          'Access token from /api/auth/login, /register, or /refresh. ' +
    ...
    schemas: {
      ...
      User: {
        ...
        properties: {
          ...
          public_key: { type: 'string', nullable: true, description: 'E2EE long-term public key (base64 X25519), null until the client has generated and uploaded one.' },
          e2ee_backup_secret: { type: 'string', nullable: true, description: 'E2EE secret key wrapped client-side with a password-derived key (PBKDF2 -> nacl.secretbox, base64). Owner-only field; opaque to the server.' },
          e2ee_backup_nonce: { type: 'string', nullable: true, description: 'secretbox nonce for e2ee_backup_secret (base64).' },
          e2ee_backup_salt: { type: 'string', nullable: true, description: 'PBKDF2 salt for the backup wrap key (base64).' },
          e2ee_backup_iters: { type: 'integer', nullable: true, description: 'PBKDF2 iteration count used for this backup.' },
        },
      },
      UserSummary: {
        type: 'object',
        ...
        allOf: [
          {
            ...
            properties: {
              ...
              status: { type: 'string', enum: ['online', 'offline'] },
              presence: { type: 'string', enum: ['online', 'away', 'busy'] },
              public_key: { type: 'string', nullable: true, description: 'E2EE long-term public key (base64 X25519), needed to send this user an encrypted direct message.' },
      ...
      Message: {
        ...
        properties: {
          ...
          is_encrypted: { type: 'boolean', description: 'True for E2EE direct-chat text messages — `text` holds base64 ciphertext, not plaintext, and nonce/sender_public_key are required to decrypt it client-side.' },
          nonce: { type: 'string', nullable: true, description: 'base64, only present when is_encrypted is true.' },
          sender_public_key: { type: 'string', nullable: true, description: 'Sender\'s public key at time of send (base64), only present when is_encrypted is true.' },
