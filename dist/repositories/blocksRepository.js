...
function findPairsInvolving(userId) {
...
function findPairBetween(userIdA, userIdB) {
    return supabaseAdmin
        .from('blocks')
        .select('blocker_id, blocked_id')
...
function existsBetween(userIdA, userIdB) {
    return supabaseAdmin
        .from('blocks')
        .select('id')
        .or(`and(blocker_id.eq.${userIdA},blocked_id.eq.${userIdB}),and(blocker_id.eq.${userIdB},blocked_id.eq.${userIdA})`)
...
function listBlockedByUser(blockerId) {
    return supabaseAdmin
        .from('blocks')
...
function insertBlock({ id, blockerId, blockedId, createdAt }) {
    return supabaseAdmin
        .from('blocks')
        .upsert({ id, blocker_id: blockerId, blocked_id: blockedId, created_at: createdAt }, { onConflict: 'blocker_id,blocked_id', ignoreDuplicates: true });
}
...
//# sourceMappingURL=blocksRepository.js.map