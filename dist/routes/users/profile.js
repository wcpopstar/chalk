 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.patch('/me', requireAuth, profileWriteLimiter, validate({ body: updateProfileSchema }), async (req, res) => {
    ...
    const { data, error } = await usersRepository.updateProfile(req.user.id, updates, 'id, username, country, languages, avatar_emoji, avatar_url, age, gender, bio, presence');
    if (error)
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.post('/me/onboarding', requireAuth, profileWriteLimiter, validate({ body: onboardingSchema }), async (req, res) => {
    ...
    const { data: user, error } = await usersRepository.updateProfile(req.user.id, updates, 'id, username, email, country, languages, avatar_emoji, avatar_url, age, gender, onboarding_completed, bio, presence');
    if (error)
        return res.status(500).json({ error: error.message });
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.put('/me/games', requireAuth, profileWriteLimiter, validate({ body: updateGamesSchema }), async (req, res) => {
    try {
 ...
 */
router.get('/me/stats', requireAuth, statsReadLimiter, async (req, res) => {
