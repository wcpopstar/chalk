 ...
 */
router.get('/discover', requireAuth, validate({ query: discoverQuerySchema }), async (req, res) => {
...
router.get('/search', requireAuth, searchLimiter, validate({ query: searchQuerySchema }), async (req, res) => {
    ...
    const ranked = (users || [])
        ...
        if (name === q)
            ...
            rank = 2; // word-ish/other substring
        return { u, rank };
    })
