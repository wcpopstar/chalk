...
router.post('/direct', requireAuth, dmLimiter, validate({ body: createDirectSchema }), async (req, res) => {
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.post('/group', requireAuth, groupLimiter, validate({ body: createGroupSchema }), async (req, res) => {
    ...
    await supabaseAdmin.from('conversation_members').insert(allMembers.map(user_id => ({ conversation_id: convId, user_id })));
    res.status(201).json({ conversation: conv });
});
/**
 * @openapi
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.get('/global/messages', requireAuth, readLimiter, validate({ query: messagesQuerySchema }), async (req, res) => {
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.get('/:id/messages', requireAuth, readLimiter, validate({ params: uuidParam(), query: messagesQuerySchema }), async (req, res) => {
    const { limit, before } = req.query;
    const uid = req.user.id;
 ...
 *             schema: { $ref: '#/components/schemas/Error' }
 */
router.get('/:id/members', requireAuth, readLimiter, validate({ params: uuidParam() }), async (req, res) => {
    const { data: member } = await supabaseAdmin
        .from('conversation_members')
        .select('user_id')
