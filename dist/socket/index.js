...
function initSocket(io) {
    ...
    io.on('connection', async (socket) => {
        ...
        safeAsync(() => supabaseAdmin.from('users').update({ status: 'online', last_seen: new Date().toISOString() }).eq('id', userId), { label: 'mark user online in DB', context: { userId } });
        ...
        socket.on('disconnect', async () => {
            ...
            safeAsync(() => supabaseAdmin.from('users').update({ status: 'offline', last_seen: new Date().toISOString() }).eq('id', userId), { label: 'mark user offline in DB', context: { userId } });
