...
async function promoteRoomToFriends(participantIds) {
    ...
    for (let i = 0; i < ids.length; i++) {
        for (let j = i + 1; j < ids.length; j++) {
            try {
                await addFriendPairInstant(ids[i], ids[j]);
            }
            catch (err) {
                logger.error({ err, userA: ids[i], userB: ids[j] }, 'Failed to add friend pair during trial promotion');
            }
        }
    }
    ...
    try {
        if (ids.length === 2) {
            ...
            if (convErr)
                throw convErr;
            await supabaseAdmin.from('conversation_members').insert(ids.map(user_id => ({ conversation_id: convId, user_id })));
            return convId;
        }
...
function startMatchLoop(io) {
    const interval = setInterval(async () => {
        try {
            ...
            if (soloMatch)
                await handleMatch(io, soloMatch, 'solo');
            if (groupMatch)
                await handleMatch(io, groupMatch, 'group');
