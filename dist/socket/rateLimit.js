...
async function isFlooding(socket, key, windowMs, max) {
...
async function isFloodingUser(userId, key, windowMs, max) {
...
async function isFloodingGlobal(userId) {
...
function clearRateLimitsFor(_socket) { }
...
async function checkConnectionBudget(socket) {
...
function resolveNamedLimit(eventName) {
    if (eventName === 'match:join')
        return { limitKey: 'match:join', ...NAMED_LIMITS['match:join'] };
    if (eventName === 'chat:message')
        return { limitKey: 'chat:message', ...NAMED_LIMITS['chat:message'] };
...
async function checkNamedLimit(userId, eventName) {
