 ...
 * shutdown, state.ts).
 */
async function checkSlidingWindow(key, windowMs, limit, warnRatio = 0.8) {
    const now = Date.now();
    const warnThreshold = Math.max(1, Math.ceil(limit * warnRatio));
    const member = `${now}-${Math.random().toString(36).slice(2, 10)}`;
    try {
        const [count, allowed, warn] = await redis.slidingWindowCheck(`${NS}:${key}`, `${NS}:${key}:warn`, now, windowMs, limit, warnThreshold, member);
        return {
            allowed: allowed === 1,
            warn: warn === 1,
            count,
