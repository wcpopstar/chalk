...
const { getRoom, deleteRoom, updateRoom, roomSize, getOnlineSocket, getUserCurrentRoom, setUserRoom, clearUserRoom, addPendingInvite, consumePendingInvite, addPendingJoinRequest, consumePendingJoinRequest, markCallPartners, } = require('./state');
...
function registerCallHandlers(io, socket, userId, username) {
    ...
    secureOn(io, socket, userId, 'call:accept', async ({ roomId, inviterId }) => {
        if (!(await consumePendingInvite(roomId, userId, inviterId)))
            return;
        ...
        await updateRoom(roomId, (room) => {
            if (!room)
                return { participants: [inviterId, userId], mode: 'direct', votes: {} };
    ...
    secureOn(io, socket, userId, 'call:request_join', async ({ targetUserId }) => {
        ...
        if (!room)
            return socket.emit('call:join_failed', { reason: 'Звонок уже завершён' });
        const { data: profile } = await supabaseAdmin
            .from('users')
            .select('id, username, avatar_emoji, avatar_url')
            .eq('id', userId)
        ...
        await Promise.all(room.participants.map(async (pid) => {
            ...
            if (pSocket)
                io.to(pSocket).emit('call:join_requested', {
                    roomId: targetRoomId,
                    from: {
                        id: userId,
                        username: profile?.username || username,
    ...
    secureOn(io, socket, userId, 'call:join_response', async ({ roomId, requesterId, accept }) => {
        ...
        if (!accept) {
            if (requesterSocket)
                io.to(requesterSocket).emit('call:join_rejected', { roomId, by: userId });
            return;
        }
        const updatedRoom = await updateRoom(roomId, (r) => {
            if (!r)
                return null; // room vanished between our read above and now
            if (!r.participants.includes(requesterId))
                r.participants.push(requesterId);
            return r;
        });
        if (!updatedRoom)
            return; // room was torn down concurrently, nothing to join
        if (requesterSocket) {
    ...
    secureOn(io, socket, userId, 'friends:call_status', async (_payload, ack) => {
        ...
        await Promise.all((friendRows || []).map(async (row) => {
            ...
            if (roomId)
                result[friendId] = { inCall: true, roomSize: await roomSize(roomId) };
        }));
        ack(result);
    });
}
