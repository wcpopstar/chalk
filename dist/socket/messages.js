...
async function saveMessage({ conversationId, senderId, text, type, mediaUrl, duration, preview }) {
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error) {
        logger.error({ err: error, conversationId, senderId }, 'Failed to save message');
...
async function saveGlobalMessage({ senderId, text, type, mediaUrl, duration, preview }) {
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error) {
        logger.error({ err: error, senderId }, 'Failed to save global message');
        throw new Error(error.message || 'Не удалось отправить сообщение');
    }
    return data;
}
...
async function editMessageRow(table, select, id, senderId, text) {
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error) {
        logger.error({ err: error, table, id, senderId }, 'Failed to edit message');
        throw new Error(error.message || 'Не удалось отредактировать сообщение');
    }
    if (!data)
...
async function deleteMessageRow(table, id, senderId) {
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error) {
        logger.error({ err: error, table, id, senderId }, 'Failed to delete message');
