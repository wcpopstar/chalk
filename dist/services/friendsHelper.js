 ...
 */
async function addFriendPairInstant(userIdA, userIdB) {
    if (!userIdA || !userIdB)
    ...
    const { error: upsertErr } = await supabaseAdmin
        .from('friends')
        .upsert({ id: uuid(), user_a: a, user_b: b, status: 'accepted', created_at: new Date().toISOString() }, { onConflict: 'user_a,user_b', ignoreDuplicates: true });
    if (upsertErr)
        throw upsertErr;
