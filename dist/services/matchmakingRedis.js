...
async function enqueue(entry) {
    ...
    if (!userId)
        throw new Error('matchmakingRedis.enqueue: userId is required');
    if (mode !== 'solo' && mode !== 'group') {
        throw new Error(`matchmakingRedis.enqueue: unknown mode "${mode}"`);
    }
    if (!gameId)
        throw new Error('matchmakingRedis.enqueue: gameId is required');
...
async function tryMatchGroup(gameId, squadSize) {
    ...
    if (q.length < squadSize)
        return null;
    const anchor = q[0];
    const group = [anchor];
    for (const entry of q.slice(1)) {
        if (group.length >= squadSize)
            break;
...
async function runMatchCycle(io) {
    ...
    if (!gotLock)
        return { soloMatch: null, groupMatch: null, matches: [] };
...
async function queueSize(mode, gameId) {
    ...
    const queues = await activeQueues();
    const sizes = await Promise.all(queues.map(async (q) => ({ ...q, size: await redis.scard(queueSetKey(q.mode, q.gameId)) })));
    const totals = { solo: 0, group: 0 };
    for (const q of sizes)
        totals[q.mode] = (totals[q.mode] || 0) + q.size;
...
//# sourceMappingURL=matchmakingRedis.js.map